...
Component.register('trustpayments-settings', {

    template: template,

    inject: [
        'TrustPaymentsConfigurationService'
    ],
    ...
    methods: {
        ...
        registerWebHooks() {
            this.TrustPaymentsConfigurationService.registerWebHooks(this.$refs.configComponent.selectedSalesChannelId)
                .then(() => {
                    this.createNotificationSuccess({
                        title: this.$tc('trustpayments-settings.settingForm.titleSuccess'),
                        message: this.$tc('trustpayments-settings.settingForm.messageWebHookUpdated')
                    });
                }).catch(() => {
                this.createNotificationError({
                    title: this.$tc('trustpayments-settings.settingForm.titleError'),
                    message: this.$tc('trustpayments-settings.settingForm.messageWebHookError')
                });
                this.isLoading = false;
            });
        },
        ...
        synchronizePaymentMethodConfiguration() {
            this.TrustPaymentsConfigurationService.synchronizePaymentMethodConfiguration(this.$refs.configComponent.selectedSalesChannelId)
                .then(() => {
                    this.createNotificationSuccess({
                        title: this.$tc('trustpayments-settings.settingForm.titleSuccess'),
                        message: this.$tc('trustpayments-settings.settingForm.messagePaymentMethodConfigurationUpdated')
                    });
                    this.isLoading = false;
                }).catch(() => {
                this.createNotificationError({
                    title: this.$tc('trustpayments-settings.settingForm.titleError'),
                    message: this.$tc('trustpayments-settings.settingForm.messagePaymentMethodConfigurationError')
                });
                this.isLoading = false;
            });
        },
        ...
        installOrderDeliveryStates(){
            this.TrustPaymentsConfigurationService.installOrderDeliveryStates()
                .then(() => {
                    this.createNotificationSuccess({
                        title: this.$tc('trustpayments-settings.settingForm.titleSuccess'),
                        message: this.$tc('trustpayments-settings.settingForm.messageOrderDeliveryStateUpdated')
                    });
                    this.isLoading = false;
                }).catch(() => {
                this.createNotificationError({
                    title: this.$tc('trustpayments-settings.settingForm.titleError'),
                    message: this.$tc('trustpayments-settings.settingForm.messageOrderDeliveryStateError')
                });
                this.isLoading = false;
            });
        },
        ...
        onSetPaymentMethodDefault() {
            ...
            this.TrustPaymentsConfigurationService.setTrustPaymentsAsSalesChannelPaymentDefault(
        ...
        setErrorStates() {
            const messageNotBlankErrorState = {
                code: 1,
                detail: this.$tc('trustpayments-settings.messageNotBlank')
            };
