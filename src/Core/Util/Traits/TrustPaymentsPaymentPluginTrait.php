...
namespace TrustPaymentsPayment\Core\Util\Traits;
...
/**
 * Trait TrustPaymentsPaymentPluginTrait
 *
 * We use a trait keep the plugin class clean and free of auxiliary functions.
 *
 * @package TrustPaymentsPayment\Core\Util\Traits
 */
trait TrustPaymentsPaymentPluginTrait {
	...
	protected function getPaymentMethodIds(Context $context): array
	{
		...
		$criteria          = (new Criteria())
			->addFilter(new EqualsFilter('handlerIdentifier', TrustPaymentsPaymentHandler::class));
	...
	protected function deleteUserData(UninstallContext $uninstallContext): void
	{
		...
		$query = 'ALTER TABLE `order` DROP COLUMN `trustpayments_lock`;';
