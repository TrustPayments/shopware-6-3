...
class TransactionPayload extends AbstractPayload {
	...
	public const ORDER_TRANSACTION_CUSTOM_FIELDS_TRUSTPAYMENTS_SPACE_ID       = 'trustpayments_space_id';
	public const ORDER_TRANSACTION_CUSTOM_FIELDS_TRUSTPAYMENTS_TRANSACTION_ID = 'trustpayments_transaction_id';

	public const TRUSTPAYMENTS_METADATA_SALES_CHANNEL_ID     = 'salesChannelId';
	public const TRUSTPAYMENTS_METADATA_ORDER_ID             = 'orderId';
	public const TRUSTPAYMENTS_METADATA_ORDER_TRANSACTION_ID = 'orderTransactionId';
	 ...
	 * @param \TrustPaymentsPayment\Core\Util\LocaleCodeProvider         $localeCodeProvider
	 * @param \Shopware\Core\System\SalesChannel\SalesChannelContext             $salesChannelContext
	 * @param \TrustPaymentsPayment\Core\Settings\Struct\Settings        $settings
	...
	public function get(): TransactionCreate
	{
		...
		$transactionData = [
			...
			'meta_data'              => [
				self::TRUSTPAYMENTS_METADATA_ORDER_ID             => $this->transaction->getOrder()->getId(),
				self::TRUSTPAYMENTS_METADATA_ORDER_TRANSACTION_ID => $this->transaction->getOrderTransaction()->getId(),
				self::TRUSTPAYMENTS_METADATA_SALES_CHANNEL_ID     => $this->salesChannelContext->getSalesChannel()->getId(),
			],
	...
	{
		/**
		 * @var \TrustPayments\Sdk\Model\LineItemCreate[] $lineItems
		 */
		$lineItems = [];
	 ...
	 *
	 * @var \Shopware\Core\Checkout\Order\Aggregate\OrderLineItem\OrderLineItemEntity $shopLineItem
	 * @return \TrustPayments\Sdk\Model\LineItemCreate|null
	 * @throws \Exception
	 */
	protected function createLineItem(OrderLineItemEntity $shopLineItem): ?LineItemCreate
	{
		...
		if (!empty($shopLineItem->getType()) && $shopLineItem->getType() == CustomProductsLineItemTypes::LINE_ITEM_TYPE_CUSTOMIZED_PRODUCTS) {
			...
			$taxes = $this->getCustomProductTaxes(
				...
				$this->translator->trans('trustpayments.payload.taxes'),
				$amount
			);
	...
	protected function getShippingLineItem(): ?LineItemCreate
	{
		try {
			...
			if ($amount > 0) {
				...
				$shippingName = $this->salesChannelContext->getShippingMethod()->getName() ?? $this->translator->trans('trustpayments.payload.shipping.name');
				$taxes        = $this->getTaxes(
				...
				$lineItem = (new LineItemCreate())
					->setAmountIncludingTax($amount)
					->setName($this->fixLength($shippingName . ' ' . $this->translator->trans('trustpayments.payload.shipping.lineItem'), 150))
	...
	protected function getAdjustmentLineItem(array &$lineItems): ?LineItemCreate
	{
		...
		if (abs($adjustmentPrice) != 0) {
			if ($this->settings->isLineItemConsistencyEnabled()) {
				...
				$lineItem = (new LineItemCreate())
					->setName($this->translator->trans('trustpayments.payload.adjustmentLineItem'))
	 ...
	 * @param string $id
	 *
	 * @return \TrustPaymentsPayment\Core\Api\PaymentMethodConfiguration\Entity\PaymentMethodConfigurationEntity
	 */
	protected function getPaymentConfiguration(string $id): PaymentMethodConfigurationEntity
	{
		$criteria = (new Criteria([$id]));

		return $this->container->get('trustpayments_payment_method_configuration.repository')
	...
	protected function getFailUrl(string $orderId): string
	{
		return $this->container->get('router')->generate(
			'frontend.trustpayments.checkout.recreate-cart',
			['orderId' => $orderId,],
