...
namespace TrustPaymentsPayment\Core\Api\Transaction\Service;
 ...
 *
 * @package TrustPaymentsPayment\Core\Api\Transaction\Service
 */
class TransactionService {
	/**
	...
	): string
	{
		...
		$this->addTrustPaymentsTransactionId(
			$transaction,
		...
		$redirectUrl = $this->container->get('router')->generate(
			'frontend.trustpayments.checkout.pay',
	 ...
	 * @param int                                                                $trustpaymentsTransactionId
	 * @param int                                                                $spaceId
	 */
	protected function addTrustPaymentsTransactionId(
		AsyncPaymentTransactionStruct $transaction,
		Context $context,
		int $trustpaymentsTransactionId,
		int $spaceId
	): void
	{
		$data = [
			...
			'customFields' => [
				TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_TRUSTPAYMENTS_TRANSACTION_ID => $trustpaymentsTransactionId,
				TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_TRUSTPAYMENTS_SPACE_ID       => $spaceId,
			],
		];
	...
	/**
	 * Persist TrustPayments transaction
	 *
	...
	): void
	{
		try {
			...
			$orderId             = $transactionMetaData[TransactionPayload::TRUSTPAYMENTS_METADATA_ORDER_ID];
			$orderTransactionId  = $transactionMetaData[TransactionPayload::TRUSTPAYMENTS_METADATA_ORDER_TRANSACTION_ID];
	 ...
	 * @param \Shopware\Core\Framework\Context $context
	 *
	 * @return \TrustPaymentsPayment\Core\Api\Transaction\Entity\TransactionEntity
	 */
	...
	/**
	 * Read transaction from TrustPayments API
	 *
	...
	/**
	 * Get transaction entity by TrustPayments transaction id
	 *
	 * @param int                              $transactionId
	 * @param \Shopware\Core\Framework\Context $context
	 *
	 * @return \TrustPaymentsPayment\Core\Api\Transaction\Entity\TransactionEntity|null
	 */
	 ...
	 * @param \Shopware\Core\Framework\Context $context
	 *
	 * @return \TrustPaymentsPayment\Core\Api\Refund\Entity\RefundEntityCollection
	 */
